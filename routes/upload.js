...
// ── Google auth (service account from env) ───────────────────────
function getAuth() {
  let key;
  try {
...
router.post('/submit', uploadLimiter, async (req, res) => {
  try {
    ...
    if (maplink && !maplink.startsWith('https://'))   errors.push('Map link must be a valid https URL.');
    ...
    for (let i = 0; i < rawImages.length; i++) {
      ...
      const filename = `${name}_${category}_${Date.now()}_${i+1}.${ext}`
      ...
      const file = await drive.files.create({
        ...
        media: { mimeType: img.type, body: stream },
      ...
      imgLinks.push(file.data.webViewLink);
    }
    ...
    console.log(`[upload/submit] New submission: ${category} from ${name} (${location})`);
